...
const PrivateRoute = ({ children }) => {
  ...
  if (loading) return <div style={{ paddingTop: '100px', textAlign: 'center' }}>Authenticating...</div>;
...
function MainApp() {
  ...
  return (
    <div className="app-container">
      ...
      {/* Main Content Router */}
      <div className="main-content" style={{ minHeight: '80vh', paddingBottom: '40px' }}>
        <Routes>
      ...
      {/* Footer */}
      <footer style={{ background: '#111', color: '#fff', padding: '40px 0', marginTop: 'auto' }}>
        <div className="container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontWeight: "800", fontSize: '24px', letterSpacing: '-0.5px' }}>CraveBites</span>
          </div>
          <p style={{ color: '#aaa', fontSize: '14px', textAlign: 'center' }}>
            Delivering the best food & drinks in your city with precision and care.
          </p>
          <div style={{ borderTop: '1px solid #333', width: '100%', marginTop: '16px', paddingTop: '16px', textAlign: 'center', color: '#777', fontSize: '12px' }}>
            © {new Date().getFullYear()} CraveBites. All rights reserved.
          </div>
        </div>
      </footer>
            ...
            {cart.length === 0 ? (
              ...
              cart.map(item => (
                <div className="cart-item" key={item.id}>
                  <img src={item.image_url || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=200&q=80"} className="cart-item-img" alt={item.name} loading="lazy" onError={handleImageError} />
